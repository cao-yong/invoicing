var addTabs = function (options) {
    ...
    if ($("#" + id).length == 0) {
    	...
    	if (options.title == undefined) {
    		title = '<li role="presentation" id="tab_' + id + '"><a href="#' + id + '" aria-controls="' + id + '" role="tab" data-toggle="tab">' + options.stBreadcrumb;
    	}else{
        ...
        if (options.content) {
            ...
            if (options.title == undefined) {
                content = '<div role="tabpanel" class="tab-pane" id="' + id + '"><div class="breadcrumbs" id="breadcrumbs' + options.id + '" style="margin:0px 0px 0px!important"><script type="text/javascript">try{ace.settings.check("breadcrumbs" , "fixed")}catch(e){}</script>' +
                    ...
                    '</ul></div><iframe src="' + options.url + '" id="mainFrame' + options.id + '" name="mainFrame' + options.id + '" width="100%" height="1200"  frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes" onLoad="iFrameHeight(\'' + options.id + '\')"></iframe></div>';
            }else if (options.title0 == undefined) {
	                content = '<div role="tabpanel" class="tab-pane" id="' + id + '"><div class="breadcrumbs" id="breadcrumbs' + options.id + '" style="margin:0px 0px 0px!important"><script type="text/javascript">try{ace.settings.check("breadcrumbs" , "fixed")}catch(e){}</script>' +
	                '<ul class="breadcrumb"><li><i class="ace-icon fa fa-home home-icon"></i><a id="1stBreadcrumb" href="javascript:void(0)">' + options.stBreadcrumb + '</a></li>' +
